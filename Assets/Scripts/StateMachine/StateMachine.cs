...
namespace ATG.StateMachine
{
	...
	public sealed class StateMachine: IStateSwitcher
	{
		...
		/// <summary>
		/// Executes the state machine (usually invoke in Update).
		/// </summary>
		...
		/// <summary>
		/// Executes the state machine (usually invoke in FixedUpdate).
		/// </summary>
		public void FixedExecuteMachine()
		{
			if (_currentState == null) return;
        ...
        {
            /*if(type.BaseType != typeof(Statement))
				throw new ArgumentException($"{type} is not {typeof(Statement)}");*/
			...
			if (_currentState == null)
				throw new Exception ("Statements not contains " + type.GetType().ToString());
